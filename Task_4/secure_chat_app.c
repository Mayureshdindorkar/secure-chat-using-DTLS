...
int exchange_server_messages(int server_socket)
{
    ...
    printf("Connected to client: '%s'\n", inet_ntoa(msg_exc_client_addr.sin_addr));
    printf("Msg recvd from Client (%s:%d): %s\n", inet_ntoa(msg_exc_client_addr.sin_addr), msg_exc_client_addr.sin_port, buffer);
    ...
    printf("Msg sent to Client (%s:%d): %s\n", inet_ntoa(msg_exc_client_addr.sin_addr), msg_exc_client_addr.sin_port, chat_ok_reply);

    memset(buffer, 0, sizeof(buffer));
    n = recvfrom(server_socket, (char *)buffer, sizeof(buffer), 0, (struct sockaddr *)&msg_exc_client_addr, &msg_exc_client_addr_size);
    printf("Msg recv from Client (%s:%d): %s\n", inet_ntoa(msg_exc_client_addr.sin_addr), msg_exc_client_addr.sin_port, buffer);
    if (strcmp(buffer, "chat_START_SSL_NOT_SUPPORTED") == 0) // Will not be used in task 4
    {
        ...
        printf("Msg sent to Client (%s:%d): %s", inet_ntoa(msg_exc_client_addr.sin_addr), msg_exc_client_addr.sin_port, UNENCRYPTED_CHAT_ACK);
    ...
    printf("Msg sent to Client (%s:%d): %s", inet_ntoa(msg_exc_client_addr.sin_addr), msg_exc_client_addr.sin_port, chat_START_SSL_ACK);
...
void runClient(char *serverHostName)
{
    ...
    // exchange_client_messages(): returns 1 if unsecure chatting is to be performed, 0 for secure chatting
