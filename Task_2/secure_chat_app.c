...
SSL_CTX *create_server_context() // bob
{
    ...
    // SSL_CTX_set_session_id_context(ctx, (const unsigned char *)"Server", strlen("Server"));
...
void exchange_server_messages(int server_socket)
{
    struct timeval timeout;
    timeout.tv_sec = 1;
    timeout.tv_usec = 0;
    if (setsockopt(server_socket, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) < 0) {
        perror("Applying recv timeout on socket failed");
        exit(EXIT_FAILURE);
    }
    ...
    do
    {
        n = recvfrom(server_socket, buffer, sizeof(buffer), 0, (struct sockaddr *)&msg_exc_client_addr, &msg_exc_client_addr_size);
    } while(n < 0);
    printf("\nMsg recvd from Client: %s\n", buffer);
    ...
    while(1)
    {
        ...
        if (n < 0) {
            printf("Packet lost. Resent '%s' message to client....\n", chat_ok_reply);
            sendto(server_socket, (const char *)chat_ok_reply, strlen(chat_ok_reply), 0, (const struct sockaddr *)&msg_exc_client_addr, sizeof(msg_exc_client_addr));
            continue;
        } else {
            if (strcmp(buffer, "chat_hello") == 0) {
    ...
    while(1)
    {
        memset(buffer, 0, sizeof(buffer));
        n = recvfrom(server_socket, (char *)buffer, sizeof(buffer), 0, (struct sockaddr *)&msg_exc_client_addr, &msg_exc_client_addr_size);
        if((n > 0) && (strcmp(buffer, "chat_START_SSL") == 0)) {
            // received 'chat_START_SSL', now resend the 'chat_START_SSL_ACK' msg to client
            sendto(server_socket, (const char *)chat_START_SSL_ACK, strlen(chat_START_SSL_ACK), 0, (const struct sockaddr *)&msg_exc_client_addr, sizeof(msg_exc_client_addr));
            printf("Packet lost. Resent %s message to client....\n", chat_START_SSL_ACK);
            continue;
        } else {
            // Start the secure chat now
            break;
        }
    }
...
void exchange_client_messages(int client_socket, union ServerAddrUnion server_addr)
{
    struct timeval timeout;
    timeout.tv_sec = 1;
    timeout.tv_usec = 0;
    ...
    printf("\nMsg recvd from Server: %s", buffer);
    
    // removing the timeout
    timeout.tv_sec = 0;
    timeout.tv_usec = 0;
    if (setsockopt(client_socket, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout)) < 0) {
        perror("removing recv timeout on socket failed");
        exit(EXIT_FAILURE);
    }
}
...
void runServer()
{
    ...
    while (1) // Handle Multiple clients
    {
        ...
        // Exchanging messages
        printf("Server started listening for connection....\n");
        exchange_server_messages(server_socket);
        printLine();
        ...
        while (res < 1) // for waiting till gettig clientHello
        {
            ...
            if (res < 0)
            {
                perror("Error occurred in DTLSv1_listen()"); //<--- "Destination address required"*
                exit(EXIT_FAILURE);
            }
        }
        ...
        do
        {
            ret = SSL_accept(ssl); // At this function call, DTLS ahndshake occurs and callback function 'ssl_info_callback' gets called.
        } while (ret == 0);
        if (ret < 0)
        {
...
void runClient(char *serverHostName)
{
    ...
    exchange_client_messages(client_socket, server_addr);
    printLine();
